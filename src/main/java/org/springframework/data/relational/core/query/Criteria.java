...
/**
 * Central class for creating queries. It follows a fluent API style so that you can easily chain
 * together multiple criteria. Static import of the {@code Criteria.property(…)} method will improve
 * readability as in {@code where(property(…).is(…)}.
 *
 * <p>The Criteria API supports composition with a {@link #empty() NULL object} and a {@link
 * #from(List) static factory method}. Example usage:
 *
 * <pre class="code">
...
public class Criteria implements CriteriaDefinition {
  ...
  private Criteria(
      @Nullable Criteria previous,
      Combinator combinator,
      List<CriteriaDefinition> group,
  ...
  private Criteria(
    ...
    if (value instanceof String strValue) {
      this.value = strValue.replace(SINGLE_QUOTE, ESCAPED_SINGLE_QUOTE);
    } else {
      this.value = value;
    }
  ...
  private Criteria(
      @Nullable Criteria previous, Combinator combinator, List<CriteriaDefinition> group) {
  ...
  /**
   * Create a new {@link Criteria} and combine it as group with {@code AND} using the provided
   * {@link List Criterias}.
   *
  ...
  /**
   * Create a new {@link Criteria} and combine it with {@code AND} using the provided {@code column}
   * name.
   *
   * @param column Must not be {@literal null} or empty.
  ...
  /**
   * Create a new {@link Criteria} and combine it with {@code OR} using the provided {@code column}
   * name.
   *
   * @param column Must not be {@literal null} or empty.
   * @return a new {@link CriteriaStep} object to complete the next {@link Criteria}.
  ...
  /**
   * Create a new {@link Criteria} and combine it as group with {@code OR} using the provided {@link
   * Criteria} group.
   *
   * @param criteria criteria object.
   * @return a new {@link Criteria} object.
  ...
  /**
   * @return the previous {@link Criteria} object. Can be {@literal null} if there is no previous
   *     {@link Criteria}.
   * @see #hasPrevious()
   */
  @Nullable
  ...
  private void unrollGroup(
      List<? extends CriteriaDefinition> criteria, StringBuilder stringBuilder) {
    ...
    for (CriteriaDefinition criterion : criteria) {
      ...
      if (!first) {
        Combinator combinator =
            criterion.getCombinator() == Combinator.INITIAL
                ? Combinator.AND
  ...
  private void render(CriteriaDefinition criteria, StringBuilder stringBuilder) {
    ...
    stringBuilder
        .append(criteria.getColumn().toSql(IdentifierProcessing.NONE))
        .append(' ')
